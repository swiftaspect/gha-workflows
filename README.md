...
| `app-token-repositories` | string | `''` | Comma-separated repositories under the caller's owner that the check run needs to read. Example: a private sibling whose release assets `make check` downloads. When set, the workflow mints a GitHub App installation token and passes it to the make steps as `GH_TOKEN`. `gh` prefers `GH_TOKEN` over `GITHUB_TOKEN`, so npm and registry flows keep the default token. |
